#!/usr/bin/env node
/**
 * Shravan Enterprises - Remaining Image Products Seed Script
 *
 * This script seeds ONLY the remaining products from the uploaded image.
 * Excluded because it already exists in your old seed list:
 * - Masking Tape
 *
 * Required env:
 *   MONGODB_URI
...
function inferGrade(productName) {
  ...
  if (/Perforated/i.test(name) && !/Non Perforated/i.test(name)) {
...
const categories = imageProductGroups.map((group, index) => {
  ...
  return {
    ...
    seoDescription: `${
      categoryDescriptions[group.category] ||
      `Quality ${group.category.toLowerCase()} products`
    } Supplied by Shravan Enterprises.`,
    status: "active",
    isActive: true,
    sortOrder: index + 100,
  ...
  return group.products.map((productName) => {
    ...
    return {
      ...
      source: {
        ...
        reviewNote: "Seeded from uploaded image product list. Verify product image later.",
      },
    };
  });
});
...
async function attachProductsToCategories(categoryCollection, categoryToProductIds) {
  ...
  for (const [categoryIdString, payload] of categoryToProductIds.entries()) {
    ...
    const oldProductIds = Array.isArray(existing?.productIds)
      ? existing.productIds
      : [];

    const oldProductIdStrings = Array.isArray(existing?.productIdStrings)
      ? existing.productIdStrings
      : [];

    const oldProducts = Array.isArray(existing?.products)
      ? existing.products
      : [];

    const oldProductSlugs = Array.isArray(existing?.productSlugs)
...
async function seed() {
  ...
  for (const product of products) {
    ...
    if (!categoryDoc?._id) {
      console.warn(
        `⚠️ Category not found for product: ${getProductName(product)} (${product.categoryName})`
      );
      continue;
    }
    ...
    if (result.skipped) {
      skippedProducts += 1;
      console.log(`⏭️ Skipped existing product: ${getProductName(product)}`);
      continue;
    }
  ...
  if (recoveredFromSlugConflicts) {
    console.log(`Recovered from existing slug conflicts: ${recoveredFromSlugConflicts}`);
  }

  console.log("\nNow restart your backend and refresh admin panel.");
}
